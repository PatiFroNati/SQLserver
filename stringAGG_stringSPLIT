...
WITH cte_tags -- seperate all words in the comments into a column
AS (
	SELECT post_id
		,LOWER(value) AS tag
	FROM Posts
...
,cte_match  --obtain distinct values and join the keywords
AS (
	SELECT DISTINCT c.post_id
		,k.topic_id
	FROM cte_tags c
	LEFT JOIN Keywords k ON c.tag = LOWER(k.word)
	GROUP BY c.post_id
		,k.topic_id
	)
    
SELECT post_id   -- aggregate into strings grouped by post_id
	,COALESCE(STRING_AGG(topic_id, ',') WITHIN GROUP (
			ORDER BY topic_id
			), 'Ambiguous!') AS 'topic'
FROM cte_match
GROUP BY post_id
